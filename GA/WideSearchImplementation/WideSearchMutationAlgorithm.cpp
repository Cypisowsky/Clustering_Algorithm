//
// Created by Michal on 25.01.2025.
//

#include "WideSearchMutationAlgorithm.h"

#include "WideSearchParentChooser.h"

void WideSearchMutationAlgorithm::mutate(Individual *individual, double chance) {

    auto result = individual->getResult();
    if(!result->mutate(chance)) {
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> chanceDist(0.0, 1.0);
        std::uniform_int_distribution<> groupDist(0, result->groupCount());

        bool done = false;
        int i=0;
        auto limit = individual->getFitness();

        while (!done && i < iterationLimit) {
            for (int i = 0; i < result->size(); i++) {
                if (chanceDist(gen) < chance) {
                    result->set(i, groupDist(gen));
                }
            }
            i++;
            if(result->getFitness() < limit)
                done = true;
        }
    }
}
